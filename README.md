...
> StylePulse 웹 서비스의 Graph RAG · Gemini Multimodal · OASIS 시뮬레이션 코어를 **FastMCP 서버 10개 툴**로 노출하고, **6개 에이전트 스킬**이 ReAct 워크플로로 오케스트레이션하는 Codex/Cursor 플러그인입니다. 마케터가 제품 이미지·UGC 영상을 채팅에 첨부하고 "한국에서 이 광고 내도 될까?"라고만 요청하면, 에이전트가 MCP 툴 체인을 자율 호출해 **리스크 방어**와 **바이럴 기회 발굴**을 한 세션 안에서 완결합니다.

**🔄 에이전트 워크플로우 (Dual Pipeline)**

웹 UI 없이도 에이전트가 스킬 문서(`src/skills/`)를 읽고 필요한 MCP 툴만 순차·병렬 호출합니다.

```
[입력] 제품 이미지/UGC 영상 + 타겟 지역 + SKU
   │
   ├─ 🛡️ 리스크 검증 체인 (방어)
   │    ad-creative-preflight → cs-risk-simulation → compliance-scan → preflight-report
   │    └ build_market_graph → generate_consumer_personas → run_cs_risk_simulation
   │       → search_recent_enforcement_cases → insight_forge → (선택) generate_ad_image
   │
   └─ 🚀 바이럴 예측 체인 (공격)
        viral-trend-forecast
        └ build_market_graph → generate_consumer_personas → run_viral_simulation
           → forecast_trend_fit → interview_personas
```

| MCP 툴 | 역할 |
|:---|:---|
| `build_market_graph` | 멀티모달 자산 → Gemini + Grounding → Neo4j 온톨로지 자동 구축 |
| `search_market_graph` | SKU×지역별 기존 그래프 조회·중복 방지 |
| `insight_forge` | Graph RAG 하이브리드 검색으로 인과적 리스크·기회 추론 |
| `generate_consumer_personas` | OASIS 페르소나 군집 생성 + 불만/바이럴 패턴 Few-shot 주입 |
| `run_cs_risk_simulation` | CS 리스크 스트레스 테스트 (비동기 폴링·라이브 중계) |
| `run_viral_simulation` | 긍정 확산 시뮬레이션 + Viral Coefficient 정량화 |
| `search_recent_enforcement_cases` | FDA·PMDA·식약처 등 **최신** 행정처분 사례 실시간 검색 |
| `interview_personas` | 시뮬레이션 핵심 페르소나 1:1 심층 인터뷰 (IPC) |
| `forecast_trend_fit` | 성분·비주얼·인플루언서·커뮤니티 4축 트렌드 적합성 분석 |
| `generate_ad_image` | 리스크 완화형 준수(compliant) 대체 광고 시안 자동 생성 |

**🛠️ 기술적 해결 (Solution)**
- **10 MCP 툴 + 6 스킬의 표준화된 인터페이스**: Cursor/Codex 등 MCP 호환 에이전트가 동일 프로토콜로 전 파이프라인을 호출 — 백엔드(`backend/app`) 코드는 **한 줄도 수정하지 않는** Zero-Pollution Wrapping.
- **비동기 폴링 프로토콜**: `run_cs_risk_simulation` / `run_viral_simulation`이 `max_wait_seconds` 단위로 중간 SimMemory 상태를 스트리밍 반환해, 에이전트가 "⏳ 3라운드 — 저자극 불만 4건 누적"처럼 진행 상황을 사용자에게 실시간 중계합니다.
- **`{region}_{sku_id}` 멀티테넌트 격리**: 로컬 별칭 레지스트리(`graph_registry.json`)로 `kr_mist`, `jp_shumaji` 등 SKU×지역별 Neo4j 그래프·시뮬레이션 세션을 완전 분리합니다.

**🎯 MCP 전용 핵심 기능 (웹 UI 대비 차별점)**
- ✅ **양방향 시뮬레이션**: CS 불만 5패턴(`EFFICACY_GAP`, `THERMAL_IRRITATION` 등) vs 바이럴 촉매 5패턴(`INGREDIENT_HYPE`, `AESTHETIC_VIRAL` 등)을 페르소나 15~35%에 의도 주입 — "장밋빛"·"과도한 비관" 편향 없이 **리스크와 기회를 대칭 분석**
- ✅ **SimMemory 기반 정량 판정**: Ebbinghaus 망각 곡선 + 강화 누적은 시뮬레이션 **전체 발화**(긍정·부정·중립)에 공통 적용 — **Go/No-Go**는 부정·CS 신호의 `long_term`(레드) / `stale`(참고)를, **바이럴 분석**은 긍정 신호의 `long_term`을 각각 Viral Coefficient·트렌드 등급의 정량 근거로 사용
- ✅ **규제 + 시장의 실시간 Grounding**: 학습 데이터가 아닌 **살아있는** 행정처분 사례·성분 트렌드·SNS 화제성을 Gemini Search로 대조
- ✅ **리스크 완화형 광고 시안 루프**: `generate_ad_image`가 원본 소재를 레퍼런스로 OpenAI `gpt-image-2`(우선) / Gemini `gemini-3.1-flash-lite`(폴백) 시안 생성 → 사용자 피드백 반영 반복 수정까지 에이전트가 자율 수행
