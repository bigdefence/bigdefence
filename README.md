...
> 제품 이미지/영상과 자연어 지침만으로 온톨로지를 자동 추출하고, 뷰티/패션 특화 가상 소비자 에이전트들의 상호작용을 통해 미래 트렌드 및 소셜 피드 반응을 예측하는 **엔드투엔드 소비자 여론 시뮬레이션 플랫폼**입니다.

**🎯 핵심 기능 및 특장점**
- ✅ **멀티모달 이미지/영상 분석**: 제품 이미지, 상세페이지 배너, UGC 리뷰 비디오 등을 분석하여 제품 외관 정보와 자막/사운드 흐름을 텍스트 설명문으로 자동 추출
- ✅ **Google Search Grounding**: 이미지/영상에서 식별된 제품의 정식 명칭, 브랜드 정보, 공식 출시 가격, 핵심 성분/소재 및 실시간 온라인 대중 평판을 구글 검색으로 역추적 및 레퍼런스 카드화
- ✅ **현실 투영형 뷰티/패션 심층 페르소나 설계**: 피부 타입, 퍼스널 컬러 등 뷰티 특화 속성뿐만 아니라 **MBTI, 인지 편향(FOMO 등), 군중 동조성, 감정적 반응성** 등 현실 세계 대중의 복잡하고 비합리적인 심리적 특성을 가상 세계의 에이전트에 그대로 투영합니다. 또한 에이전트마다 **개별 활동성(activity_level)** 및 **활동 시간대(active_hours)** 를 부여해 심야 수면 등 실제 사람과 똑같은 행동 패턴을 반영함으로써, 실제와 같은 유기적인 소셜 미디어 반응(악플, 팬덤 형성 등)을 완벽하게 시뮬레이션함과 동시에 무분별한 LLM 호출(과금 누수)을 원천 차단합니다.
- ✅ **인스타 감성 소셜 피드 UI & 프리미엄 마크다운 렌더링**: 은은한 핑크 글로우 효과가 적용된 소셜 카드 피드를 제공하며, 마크다운(Markdown) 기반의 심층 분석 보고서와 대화형 채팅(Q&A) 내 모든 수치/통계 데이터를 자체 설계한 프리미엄 표(Table) 디자인으로 렌더링하여 압도적인 시각적 완성도와 가독성을 자랑합니다.
- ✅ **고도화된 프롬프트 엔지니어링 및 예외 처리**: 시각적/UI 디자인 요소(폰트, 버튼 색상 등)를 철저히 배제하고 코어 비즈니스 도메인에만 집중하도록 온톨로지 생성 프롬프트를 설계했으며, JSON 스키마 네이밍 룰(PascalCase 등)이 결과값에 오염되지 않도록 완벽히 제어했습니다.
- ✅ **하이브리드 Graph RAG (1536 dim)**: Neo4j 5.x 기반 Vector Search(1536 dim), Full-Text, Keyword 매칭을 결합하여 지식 그래프 정보 회수율을 극대화했습니다. 특히 Full-Text 쿼리 입력 시 발생할 수 있는 Lucene 문법 에러(특수문자 파싱 오류)를 백엔드에서 원천 차단하여 검색 안정성을 한 차원 높였습니다.
...
**💡 기존 서비스의 문제 정의 (Problem)**
- **평면적 여론 분석의 한계**: 기존의 시장 조사나 단순 텍스트 기반 분석은 정형화된 설문에만 의존하여, 현대 뷰티/패션 소비자의 입체적인 심리(MBTI, 인지 편향, 감정적 반응성)와 복잡한 소셜 미디어 바이럴 흐름을 전혀 담아내지 못했습니다.
- **멀티모달 마케팅 자산의 단절**: 제품의 진짜 가치를 전달하는 핵심 매체는 뷰티 화보(이미지)와 숏폼 영상(UGC 리뷰)임에도, 이를 지식 그래프(온톨로지) 및 시뮬레이션 환경으로 자동 변환해주는 AI 파이프라인이 부재했습니다.
- **실시간 시장 트렌드와의 괴리**: LLM 내부의 과거 데이터에만 갇혀 있어, 시시각각 변하는 화장품 가격 정보나 뷰티 성분 트렌드, 대중의 실시간 평판을 반영한 생동감 있는 시뮬레이션이 불가능했습니다.

**🛠️ 기술적 해결 및 핵심 성과 (Solution & Value)**
- **엔드투엔드 멀티모달 트렌드 인젝션**: 제품 이미지(`.png`)나 리뷰 숏폼(`.mp4`)을 드래그 앤 드롭하는 즉시, Gemini Multimodal과 Google Search Grounding이 시각/청각 데이터를 분석하고 최신 웹 트렌드(가격, 성분, 평판)를 병합하여 고정밀 지식 그래프(온톨로지)를 전자동으로 구축합니다.
- **현실 투영형 하이브리드 Graph RAG 시뮬레이션**: 1536차원 벡터와 Full-Text 검색이 결합된 Neo4j 기반 RAG를 활용해, 현실 세계 대중의 비합리적 심리와 소비 성향을 완벽히 투영한 에이전트들이 가상의 인스타/트위터/레딧 공간에서 유기적으로 소셜 여론을 형성(팬덤, 악플 등)하는 디지털 샌드박스를 구현했습니다.
- **IPC 기반 1:1 심층 인터뷰를 통한 마케팅 리허설**: 단순한 통계 보고서를 넘어, 시뮬레이션에서 여론을 주도했던 핵심 인플루언서나 불만 소비자 에이전트를 실시간으로 소환해 양방향 심층 인터뷰(Q&A)를 진행함으로써 정성적인 브랜드 마케팅 인사이트를 획득합니다.
- **완벽한 리얼리즘과 API 토큰 최적화의 결합**: 에이전트마다 고유의 활동성(activity_level)과 수면 시간대(active_hours)를 엄격하게 부여해 실제 인간의 라이프사이클을 반영했습니다. 이를 통해 시뮬레이션의 리얼리티를 극대화함과 동시에, 병렬 처리 시 발생하는 불필요한 LLM 과금 누수(Token Leak)를 원천 차단했습니다. 

**🔧 기술적 구현 및 시스템 아키텍처**
- **데이터 최적화**: Neo4j 5.x의 Vector Index와 Full-Text Index를 하이브리드로 운용하여 대규모 지식 구조에서도 밀리초 단위의 시맨틱 조회 보장
- **프런트엔드**: Vue 3 + Vite 기반의 세련된 UI (Outfit / Playfair Display / Inter 폰트, 딥 바이올렛 & 로즈 핑크 & 골드 테마 적용)
- **Frontend / Backend**: Vue 3 · D3 · Python FastAPI · Neo4j Bolt · OASIS (CAMEL-AI) · Google GenAI
