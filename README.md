...
**2026.03 - 현재** | [Demo](https://www.youtube.com/watch?v=lKeVcfvjavs)

[![StylePulse Demo](https://img.youtube.com/vi/lKeVcfvjavs/maxresdefault.jpg)](https://youtu.be/lKeVcfvjavs)
...
**💡 기존 서비스의 문제 정의 (Problem)**
- **단편적인 설문 및 여론 분석의 한계**: 기존의 시장 조사 및 트렌드 분석은 정적인 텍스트 데이터나 정형화된 설문 문항에만 의존하여, 소비자의 복잡한 내적 성향(피부 타입, 퍼스널 컬러 등)과 소셜 미디어 상의 동적인 여론 바이럴 흐름을 입체적으로 재현하기 어렵습니다.
- **멀티모달 마케팅 자산의 활용 부재**: 신제품의 강점과 가치는 상세페이지 이미지, 인플루언서 UGC 숏폼 비디오 등의 미디어를 통해 전달되지만, 기존 분석 시스템은 시각·청각 데이터를 텍스트 컨텍스트 및 마켓 온톨로지로 연계하는 자동화 파이프라인이 부재했습니다.
- **실시간 정보와의 단절**: 급변하는 뷰티/패션 시장 트렌드 및 소비자 여론을 추적하려면 실시간 가격 정보, 최신 성분 트렌드 등이 연동되어야 하나, 시뮬레이터 시스템 내부 지식에만 의존하여 정보의 신뢰도와 생동감이 부족했습니다.

**🛠️ 기술적 해결 및 핵심 성과 (Solution & Value)**
- **엔드투엔드 멀티모달 트렌드 인젝션**: 이미지(.png) 및 비디오(.mp4)를 드롭존에 넣는 것만으로, Gemini 멀티모달 API가 제품 외관 및 자막/대사 흐름을 자동 파싱하고 Google Search Grounding을 통해 실시간 제품 상세 스펙과 시장 평판을 수집하여 고정밀 온톨로지를 자동 추출합니다.
- **하이브리드 Graph RAG 기반 소셜 시뮬레이션**: Neo4j Graph 데이터베이스(1536 dim Vector + Full-text 하이브리드 RAG)를 구축하여, 정교한 페르소나가 부여된 가상 에이전트들이 트위터/레딧 레이아웃의 소셜 피드에서 게시글과 댓글로 상호작용하는 디지털 샌드박스를 실현했습니다.
- **1:1 양방향 심층 인터뷰를 통한 마케팅 리허설**: 시뮬레이션 결과를 요약한 보고서 도출에 그치지 않고, 여론을 주도한 인플루언서나 소비자 에이전트를 실시간 소환해 IPC 기반 1:1 심층 인터뷰 및 질의응답을 나눔으로써 정량 수치로는 파악할 수 없는 정성적인 마케팅 인사이트를 도출합니다.
- **운영 안정성 및 토큰 비용 최적화**: 병렬 시뮬레이션(Twitter+Reddit) 환경에서 발생하는 LLM API 과다 호출 문제를 해결하기 위해, 에이전트의 개별 활동성 및 시간대 속성을 엄격하게 적용하여 API 비용을 획기적으로 절감했습니다. 또한, Neo4j APOC 플러그인 의존성을 제거하고 백엔드 네이티브 파싱을 적용하여 데이터베이스 이식성과 시스템(UI 렌더링) 안정성을 크게 강화했습니다.
